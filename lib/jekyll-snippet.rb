...
require 'base64'
require 'jekyll-snippet/utils'

module Jekyll
  class SnippetTag < Liquid::Block
...
MSG
    ...
    def render(context)
      ...
      rendered_output = add_code_tag(output, code)
...
MSG
    ...
    def render_rouge(code)
      formatter = Jekyll::Utils::Rouge.html_formatter(
        ...
        :start_line   => @highlight_options[:start_line]&.to_i || 2,
        :wrap         => false,
        :css_class    => "highlight",
        :gutter_class => "gutter",
        :code_class   => "code"
    ...
    def add_code_tag(code, original)
      ...
      if (name = @highlight_options[:name]).is_a?(Array)
        name = name.first
      end
...
<figure class="highlight">
  <div class="snippet-header">
    <div class="snippet-name"><span>#{name}</span></div>
    <div class="snippet-actions">
      <button class="snippet-action-copy" data-snippet="#{Base64.encode64(original)}"></button>
    </div>
  </div>
  <pre><code #{code_attributes}>#{code.chomp}</code></pre>
</figure>
STRING
    end
  end
...
Liquid::Template.register_tag('highlight', Jekyll::SnippetTag)

Jekyll::Hooks.register :site, :post_write do |site|
  src = File.expand_path(File.join('..', '..', 'assets'), __FILE__)
  dest = File.join(site.dest, 'assets')
  Jekyll::Snippet::Utils.cp_r src, dest
end
